(() => {
  ...
  const state = {sandboxes: [], activeId: null, graph: {nodes: [], edges: [], documents: []}, tags: [], selected: new Set(), filters: new Set(), docFilters: new Set(), focusMode: false, focusNodes: new Set(), bankOpen: false, transform: {x: 0, y: 0, scale: 1}};
  ...
  const docById = id => (state.graph.documents || []).find(doc => String(doc.id) === String(id));
  ...
  function graphNodes() {
    ...
    const visible = nodes().filter(node => (!state.filters.size || (node.tag_ids || []).some(id => state.filters.has(String(id)))) && (!state.docFilters.size || (node.documents || []).some(doc => state.docFilters.has(String(doc.id)))) && (!focused || focused.has(String(node.id))));
  ...
  async function loadSandboxes(preferredId = null) {
    ...
    state.activeId = next;
    state.selected.clear(); state.filters.clear(); state.docFilters.clear();
  ...
  async function loadGraph(sandboxId = state.activeId) {
    ...
    state.graph = {nodes: data.nodes, edges: data.edges, documents: data.documents || []};
  ...
  function render() {
    ...
    renderFilter(); renderFocusMode(); renderGraph(); renderSelection(); renderDetail(); renderBank();
  ...
  function renderFilter() {
    ...
    const filtered = state.filters.size || state.docFilters.size;
    $('#filterMenu').innerHTML = `<button class="filter-all filter-check" data-all-filter><span class="custom-checkbox ${filtered ? '' : 'checked'}"><i data-lucide="check"></i></span>All topics</button><h4>Tags</h4>${allTags.map(tag => `<label class="filter-check"><input type="checkbox" data-filter-tag="${tag.id}" ${state.filters.has(String(tag.id)) ? 'checked' : ''}><span class="custom-checkbox"><i data-lucide="check"></i></span>${escape(tag.name)}</label>`).join('')}${docs.length ? `<h4>Documents</h4>${docs.map(doc => `<label class="filter-check"><input type="checkbox" data-filter-doc="${doc.id}" ${state.docFilters.has(String(doc.id)) ? 'checked' : ''}><span class="custom-checkbox"><i data-lucide="check"></i></span><i data-lucide="file-text" class="filter-doc-icon"></i>${escape(doc.name)}</label>`).join('')}` : ''}<button class="reset-filter ${filtered ? '' : 'hidden'}" data-reset-filter>Reset filters</button>`;
  }
  ...
  function renderDetail() {
    ...
    $('#detailDocuments').innerHTML = (one.documents.length ? one.documents.map(document => `<li><a class="doc-link" href="${escape(document.url)}" target="_blank" rel="noopener"><i data-lucide="file-text"></i><span>${escape(document.name)}</span></a><button class="doc-delete" data-delete-doc="${document.id}" aria-label="Remove ${escape(document.name)}"><i data-lucide="x"></i></button></li>`) : ['<li>No documents attached</li>']).join('');
  ...
  document.addEventListener('click', event => {
    ...
    const sandbox = event.target.closest('[data-sandbox]'); if (sandbox) { state.activeId = Number(sandbox.dataset.sandbox); state.selected.clear(); state.filters.clear(); state.docFilters.clear(); loadGraph(state.activeId).catch(error => showError(error.message)); return; }
  ...
  document.addEventListener('change', event => { if (event.target.matches('[data-filter-tag]')) { const tag = tagById(event.target.dataset.filterTag); if (!tag) return; event.target.checked ? state.filters.add(String(tag.id)) : state.filters.delete(String(tag.id)); render(); } if (event.target.matches('[data-filter-doc]')) { const id = String(event.target.dataset.filterDoc); event.target.checked ? state.docFilters.add(id) : state.docFilters.delete(id); render(); } });
  ...
  let dropTarget = null;
  const clearDropTarget = () => { dropTarget?.classList.remove('drop-target'); dropTarget = null; };
  document.addEventListener('dragstart', event => { const doc = event.target.closest('[data-bank-doc]'); if (doc) { event.dataTransfer.setData('application/x-atlas-doc', doc.dataset.bankDoc); event.dataTransfer.effectAllowed = 'link'; } });
  graphSvg.addEventListener('dragover', event => { const group = event.target.closest('[data-node]'); if (group && event.dataTransfer.types.includes('application/x-atlas-doc')) { event.preventDefault(); event.dataTransfer.dropEffect = 'link'; if (dropTarget !== group) { clearDropTarget(); dropTarget = group; group.classList.add('drop-target'); } } else clearDropTarget(); });
  graphSvg.addEventListener('dragleave', () => clearDropTarget());
  graphSvg.addEventListener('drop', event => { const group = event.target.closest('[data-node]'), docId = event.dataTransfer.getData('application/x-atlas-doc'); clearDropTarget(); if (!group || !docId) return; event.preventDefault(); const doc = docById(docId), topicId = Number(group.dataset.node); if (!doc) return; const topicIds = new Set((doc.topic_ids || []).map(Number)); if (topicIds.has(topicId)) return; workspaceApi.updateDocument(doc.id, {topic_ids: [...topicIds, topicId]}).then(() => refreshGraph()).catch(error => showError(error.message)); });

  const bankDrop = $('#bankDrop'), uploadToBank = async file => { if (!file || !state.activeId) return; const row = document.createElement('div'); row.className = 'bank-doc pending'; row.innerHTML = `<i data-lucide="loader"></i><span class="bank-doc-name">${escape(file.name)}</span><span class="bank-doc-count">Assigning…</span>`; $('#bankList').prepend(row); if (window.lucide) window.lucide.createIcons(); try { await workspaceApi.uploadDocument(state.activeId, file, []); await refreshGraph(); } catch (error) { row.remove(); showError(error.message); } };
  $('#bankFile').addEventListener('change', event => { [...event.target.files].forEach(uploadToBank); event.target.value = ''; });
  bankDrop.addEventListener('dragover', event => { if (event.dataTransfer.types.includes('Files')) { event.preventDefault(); bankDrop.classList.add('is-dragover'); } });
  bankDrop.addEventListener('dragleave', () => bankDrop.classList.remove('is-dragover'));
  bankDrop.addEventListener('drop', event => { event.preventDefault(); bankDrop.classList.remove('is-dragover'); [...event.dataTransfer.files].forEach(uploadToBank); });
