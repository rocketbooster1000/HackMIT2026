(() => {
  ...
  const state = {sandboxes: [], activeId: null, graph: {nodes: [], edges: []}, tags: [], selected: new Set(), filters: new Set(), transform: {x: 0, y: 0, scale: 1}};
  ...
  const tagId = name => state.tags.find(tag => tag.name === name)?.id;
  const tagIds = names => names.map(tagId).filter(Boolean);
  ...
  function graphNodes() {
    const visible = nodes().filter(node => !state.filters.size || node.tags.some(tag => state.filters.has(tag)));
    const columns = Math.max(1, Math.min(3, Math.ceil(Math.sqrt(visible.length || 1))));
    return visible.map((node, index) => ({...node, x: 230 + (index % columns) * (540 / Math.max(columns - 1, 1)), y: 105 + Math.floor(index / columns) * 145}));
  }
  ...
  function render() {
    ...
    renderFilter(); renderGraph(); renderSelection(); renderDetail();
  ...
  function renderDetail() {
    ...
    $('#detailTags').innerHTML = one.tags.map(tag => `<button class="tag tag-remove" data-remove-tag="${escape(tag)}">${escape(tag)}<i data-lucide="x"></i></button>`).join('');
  }
  ...
  const topicFields = (node = {name: '', description: '', tags: []}, context = 'topic-create') => `<div class="field"><label for="topicName">Topic name</label><input id="topicName" value="${escape(node.name)}" placeholder="e.g. Related rates" autofocus></div><div class="field"><label for="topicDescription">Description</label><textarea id="topicDescription" placeholder="What should a student understand?">${escape(node.description)}</textarea></div><div class="field"><label>Tags</label><div class="modal-tags">${state.tags.map(tag => `<button type="button" class="tag-toggle ${node.tags.includes(tag.name) ? 'active' : ''}" data-tag="${tag.id}">${escape(tag.name)}</button>`).join('')}<button type="button" class="new-tag-button" data-create-tag="${context}"><i data-lucide="plus"></i>New tag</button></div></div>`;
  function draftTopic() { return {name: $('#topicName')?.value || '', description: $('#topicDescription')?.value || '', tags: [...document.querySelectorAll('.tag-toggle.active')].map(button => tagById(button.dataset.tag)?.name).filter(Boolean)}; }
  function createNodeModal(edit = false, draft = null) {
    const node = draft || (edit ? selectedNodes()[0] : undefined); openModal(`<h2>${edit ? 'Edit topic' : 'Create node'}</h2><p class="sub">${edit ? 'Update this topic in your knowledge map.' : 'Add a concept to your knowledge map.'}</p><form id="topicForm">${topicFields(node, edit ? 'topic-edit' : 'topic-create')}<div class="modal-actions"><button type="button" class="soft-button" data-close-modal>Cancel</button><button class="primary-button">${edit ? 'Save changes' : 'Create topic'}</button></div></form>`);
    $('#topicForm').addEventListener('submit', async event => { event.preventDefault(); const name = $('#topicName').value.trim(); if (!name) return; const data = {name, description: $('#topicDescription').value.trim(), tag_ids: [...document.querySelectorAll('.tag-toggle.active')].map(button => Number(button.dataset.tag))}; try { if (edit) await workspaceApi.updateNode(node.id, data); else await workspaceApi.createNode(state.activeId, data); closeModal(); await refreshGraph(); if (!edit) state.selected = new Set([String(state.graph.nodes.at(-1)?.id)]); render(); } catch (error) { showError(error.message); } });
  }
  ...
  function changeZoom(amount) { state.transform.scale = Math.max(.55, Math.min(1.8, state.transform.scale + amount)); renderGraph(); }
  ...
  document.addEventListener('click', event => {
    ...
    const node = event.target.closest('[data-node]'); if (node) { selectNode(node.dataset.node, event); return; }
    ...
    if (event.target.closest('#addButton')) $('#addMenu').classList.toggle('hidden'); if (event.target.closest('#filterButton')) $('#filterMenu').classList.toggle('hidden');
    ...
    if (event.target.closest('#bulkTag')) tagModal(state.selected); if (event.target.closest('#deleteNode') || event.target.closest('#deleteNodes')) deleteNodesModal(); if (event.target.closest('#clearSelection') || event.target.closest('#closePanel')) { state.selected.clear(); render(); }
    ...
    const apply = event.target.closest('[data-apply-tag]'); if (apply) workspaceApi.applyTag(state.activeId, Number(apply.dataset.applyTag), [...state.selected]).then(async () => { closeModal(); await refreshGraph(); }).catch(error => showError(error.message));
    const remove = event.target.closest('[data-remove-tag]'); if (remove) { const node = selectedNodes()[0], tag = state.tags.find(item => item.name === remove.dataset.removeTag); if (node && tag) workspaceApi.removeTag(node.id, tag.id).then(refreshGraph).catch(error => showError(error.message)); }
  ...
  document.addEventListener('change', event => { if (event.target.matches('[data-filter-tag]')) { const tag = tagById(event.target.dataset.filterTag); if (!tag) return; event.target.checked ? state.filters.add(tag.name) : state.filters.delete(tag.name); render(); } });

  const graphSvg = $('#graphSvg'), selectionBox = $('#selectionBox'); let graphDrag = null;
  ...
  graphSvg.addEventListener('pointerdown', event => { if (event.button !== 0 && event.button !== 2) return; const point = pointInSvg(event); if (event.button === 2) { event.preventDefault(); graphDrag = {kind: 'pan', x: event.clientX, y: event.clientY, tx: state.transform.x, ty: state.transform.y}; graphSvg.setPointerCapture(event.pointerId); graphSvg.classList.add('dragging'); return; } if (event.target.closest('[data-node]')) return; graphDrag = {kind: 'select', start: point, current: point, additive: event.shiftKey}; graphSvg.setPointerCapture(event.pointerId); drawSelectionBox(point, point); });
  graphSvg.addEventListener('pointermove', event => { if (!graphDrag) return; if (graphDrag.kind === 'pan') { state.transform.x = graphDrag.tx + event.clientX - graphDrag.x; state.transform.y = graphDrag.ty + event.clientY - graphDrag.y; renderGraph(); } else { graphDrag.current = pointInSvg(event); drawSelectionBox(graphDrag.start, graphDrag.current); } });
  graphSvg.addEventListener('pointerup', event => { if (!graphDrag) return; const drag = graphDrag; graphDrag = null; graphSvg.classList.remove('dragging'); if (graphSvg.hasPointerCapture(event.pointerId)) graphSvg.releasePointerCapture(event.pointerId); if (drag.kind === 'select') { drag.current = pointInSvg(event); const moved = Math.abs(drag.current.x - drag.start.x) > 4 || Math.abs(drag.current.y - drag.start.y) > 4; clearSelectionBox(); if (moved) selectBoxContents(drag); else { state.selected.clear(); render(); } } });
  graphSvg.addEventListener('pointercancel', () => { graphDrag = null; clearSelectionBox(); graphSvg.classList.remove('dragging'); }); graphSvg.addEventListener('wheel', event => { event.preventDefault(); changeZoom(event.deltaY > 0 ? -.08 : .08); }, {passive: false});
